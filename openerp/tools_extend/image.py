...
def image_resize_image(base64_source, size=(1024, 1024), encoding='base64', filetype='PNG', avoid_if_small=False):
    ...
    if asked_width is None:
        asked_width = int(image.size[0] * (float(image.size[1]) / image.size[1]))
    if asked_height is None:
        asked_height = int(image.size[1] * (float(image.size[0]) / image.size[0]))
    ...
    if image.size != size:
        ...
        image.thumbnail(size, Image.ANTIALIAS)#ImageOps.fit(image, size, Image.ANTIALIAS)
...
def image_resize_image_small_map(base64_source, size=(64, 64), encoding='base64', filetype='PNG', avoid_if_small=False):
    return image_resize_image(base64_source, size, encoding, filetype, avoid_if_small)
...
def image_get_resized_images(base64_source, return_big=False, return_medium=True, return_small=True, return_small_map=True,
    big_name='image', medium_name='image_medium', small_name='image_small', small_map_name='image_small_map',
    avoid_resize_big=True, avoid_resize_medium=False, avoid_resize_small=False, size_small_map=(64, 64)):
    ...
    if return_small_map:
        return_dict[small_map_name] = image_resize_image_small_map(base64_source, avoid_if_small=avoid_resize_small, size=size_small_map)
    return return_dict
