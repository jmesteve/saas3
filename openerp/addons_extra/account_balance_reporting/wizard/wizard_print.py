...
class print_wizard(orm.TransientModel):
    ...
    def _get_current_report_id(self, cr, uid, context=None):   
        ...
        report_id = None
        if context.get('active_model') == 'account.balance.reporting' and context.get('active_ids') and context.get('active_ids')[0]:
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if data.get('report_xml_id'):
            ...
            report_xml_ids = rpt_facade.search(cr, uid, [('id', '=', report_xml_id[0])], context=context)
            report_xml_id = report_xml_ids and report_xml_ids[0] or None
            if report_xml_id:
                report_xml = rpt_facade.browse(cr, uid, [report_xml_id], context=context)[0]
            if report_xml:
                return {
